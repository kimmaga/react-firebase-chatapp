...
import {FaGrinTongueSquint, FaRegSmileWink} from 'react-icons/fa';
...
export class ChatRooms extends Component {
    ...
    //component 가 destroy 되기 직전에 만들어짐
    ...
    // 방을 만들때 데이터베이스에 들어가면 챗룸테이블에 데이터가 들어오면 바로 가져올수 있다.
    ...
    handleNotification = (chatRoomId,currentChatRoomId,notifications,DataSnapshot) => {
        ...
        //notification state 안에 해당 채팅방의 알림 정보가 없을 때
            if(index === -1){
                notifications.push({
                    id:chatRoomId, //채팅방아이디
                    total:DataSnapshot.numChildren(),
                    lastKnownTotal:DataSnapshot.numChildren(), //이전에 확인한 전체메시지 개수
                    count:0
                })
            }
            //이미 notifications안에 해당 chatRoom 데이터가 있을 때
                ...
                if(chatRoomId !== currentChatRoomId){
                    ...
                    //count 알림으로 보여줄 숫자를 구하는 부분
                    //현재 총메시지 개수 - 이전에 확인한 총 메시지 개수 > 0
